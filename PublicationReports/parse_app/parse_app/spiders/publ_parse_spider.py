...
class PublParseSpiderSpider(CrawlSpider):
    ...
    start_urls = ["https://scholar.google.com/citations?view_op=list_works&hl=ru&hl=ru&user="+id_author+"&pagesize=100"]
    ...
    def parse_item(self, response):
        ...
        publication = response.xpath('//div[@id="gsc_vcpb"]//div[@class="gsc_oci_value"]')

        titles = ['Authors', 'Year', 'journal', 'Volume', 'Number', 'Pages', 'Publisher', '', '', '']
        i = 0
        for publication in publication:
            item[titles[i]] = publication.xpath('./text()').get()
            i += 1
